...
//extern void VMC_Calc_Base_Data_Right( vmc_leg_t *vmc, INS_t *ins, float dt ); // 计算theta和d_theta给lqr用，同时也计算腿长L0
extern void VMC_Calc_Base_Data( vmc_leg_t *vmc, INS_t *ins, float dt );
extern void VMC_Calc_T_Joint( vmc_leg_t *vmc ); // 计算期望的关节输出力矩

extern void VMC_FN_Ground_Detection_R( vmc_leg_t *vmc,moving_average_filter_t *filter,float dead_line); // 右腿离地检测
extern void VMC_FN_Ground_Detection_L( vmc_leg_t *vmc,moving_average_filter_t *filter,float dead_line); // 左腿离地检测
