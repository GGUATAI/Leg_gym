...
void VMC_Calc_Base_Data( vmc_leg_t *vmc, INS_t *ins, float dt ) // 计算theta和d_theta给lqr用，同时也计算腿长L0
{
...
void VMC_FN_Ground_Detection_R( vmc_leg_t *vmc, moving_average_filter_t *filter, float dead_line )
{
	static float F_temp;
	F_temp = vmc->F_N;
...
void VMC_FN_Ground_Detection_L( vmc_leg_t *vmc, moving_average_filter_t *filter, float dead_line )
{
	static float F_temp;
	F_temp = vmc->F_N;
