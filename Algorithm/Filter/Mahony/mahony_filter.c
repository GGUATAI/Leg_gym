...
void RotationMatrix_Update(mahony_filter_t *mahony_filter)
{
...
// 从旋转矩阵中提取姿态角
void Mahony_Output(mahony_filter_t *mahony_filter)
{
...
void Mahony_Init(mahony_filter_t *mahony_filter, float Kp, float Ki, float dt)
{
    mahony_filter->Kp = Kp;
    mahony_filter->Ki = Ki;
    mahony_filter->dt = dt;
